...
def draw_parallel_lines(n, point, length, window):
    ...
    line_window = window
    first_start = point
    first_end = rg.Point(point.x+length, point.y)
    # first_line=rg.Line(first_start,first_end)
    # first_line.attach_to(line_window)
    start = first_start
    end = first_end
    line = rg.Line(start, end)
    line.attach_to(line_window)

    for k in range(n-1):
        start.y = first_start.y + 30
        print(start)
        end.y = first_end.y + 30
        line = rg.Line(start, end)
        line.attach_to(line_window)
    line_window.render()
...
def draw_lines(n, point, window):
    ...
    line_1 = rg.Line(start, end)
    line_1.attach_to(window)
    for k in range(n-1):
        step = 200/(n-1)
        new_end = end
        new_end.y = end.y - step
        line_new = rg.Line(start, new_end)
